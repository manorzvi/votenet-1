...
def dump_results(end_points, dump_dir, config, inference_switch=False):
    ...
    for i in range(batch_size):
        ...
        if 'vote_xyz' in end_points:
            pc_util.write_ply(end_points['vote_xyz'][i, :, :],
                              os.path.join(dump_dir, '%06d_vgen_pc.ply' % (idx_beg + i)))
            pc_util.write_ply(aggregated_vote_xyz[i, :, :],
        ...
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            pc_util.write_ply(pred_center[i, objectness_prob > DUMP_CONF_THRESH, 0:3],
        ...
        if np.sum(objectness_prob > DUMP_CONF_THRESH) > 0:
            ...
            if len(obbs) > 0:
                ...
                pc_util.write_oriented_bbox(obbs[objectness_prob > DUMP_CONF_THRESH, :],
                ...
                pc_util.write_oriented_bbox(obbs[pred_mask[i, :] == 1, :],
    ...
    for i in range(batch_size):
        ...
        pc_util.write_ply(gt_masked_votes[i, :, 0:3],
                          os.path.join(dump_dir, '%06d_gt_masked_votes.ply' % (idx_beg + i)))

    for i in range(batch_size):
        if np.sum(objectness_label[i, :]) > 0:
            pc_util.write_ply(pred_center[i, objectness_label[i, :] > 0, 0:3],
